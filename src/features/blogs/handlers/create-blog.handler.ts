...
export function createBlogHandler(req: TypedRequestBody<BlogInputModel>, res: Response<BlogViewModel>) {
  const { name, websiteUrl, description} = req.body;

  const newDriver: OutputBlogType = {
  ...
  res.status(HttpStatus.Created_201).send(newDriver);
}
